...
function renderLaws() {
  ...
  for (const [key, def] of Object.entries(LAWS)) {
    ...
    for (const [val, text] of Object.entries(def.options)) {
      ...
      b.textContent = text;
      b.className = world.law[key] === val ? "on" : "inert";
      b.disabled = true;
      wrap.appendChild(b);
    }
...
function renderInspect(p) {
  ...
  if (!p) { box.className = "inspect empty"; box.textContent = "Klik iemand. Het volk reguleert zichzelf."; return; }
  box.className = "inspect";
  const f = world.kinshipF(p);
  const thesis = p.thesis ? `<div class="who-line">${p.philosopher ? "denker" : "leerling"} · ${LAWS[p.thesis.key].options[p.thesis.value]}</div>` : "";
  box.innerHTML = `<div class="name">${p.name} · ${p.sex === "v" ? "vrouw" : "man"}</div>
    <div>leeftijd ${p.age}</div><div>${p.founder ? "stichter" : "nakomeling"} · kinderen ${p.kids}</div>
    <div>heterozygotie ${(world.heterozygosity(p) * 100).toFixed(0)}%</div><div>inteelt F ${f.toFixed(3)}</div>
    <div>huis ${p.house} · geslacht ${p.gen}</div><div class="who-line">${identityLine(p)}</div>${thesis}`;
}
function renderOverlay() {
  if (world.ended) {
    ...
    el("again").onclick = () => location.reload(); return;
  }
  if (world.event) {
    overlay.classList.add("show");
    overlay.innerHTML = `<div class="card omen"><p class="kicker">Denker · jaar ${world.year}</p><h3>${world.event.title}</h3><p>${world.event.body}</p><div class="choices"></div></div>`;
    ...
    world.event.choices.forEach((c, i) => {
      ...
      b.onclick = () => { world.resolve(i); renderLaws(); }; wrap.appendChild(b);
    }); return;
  }
...
function draw() {
  ...
  g.addColorStop(0, risk === "kritiek" ? "#2a1010" : "#16101c"); g.addColorStop(1, "#08070c");
  ctx.fillStyle = g; ctx.beginPath(); ctx.arc(w / 2, h / 2, w / 2 - 4, 0, Math.PI * 2); ctx.fill();
  ctx.strokeStyle = risk === "kritiek" ? "#c45a4a" : risk === "hoog" ? "#c48a4a" : "#3d3420";
  ctx.lineWidth = risk === "kritiek" ? 10 : 6; ctx.stroke();
  const live = world.alive(), sel = world.selected;
  for (const p of live) {
    let alpha = 0.78;
    if (world.showKin && sel) alpha = 0.12 + Math.min(0.88, world.shared(p, sel) * 4);
    ctx.beginPath();
    ctx.fillStyle = `hsla(${p.hue}, 62%, ${p.sex === "v" ? 68 : 58}%, ${alpha})`;
    ctx.arc(p.x * w, p.y * h, p.philosopher ? 4.2 : p.age < 12 ? 2.2 : 2.8, 0, Math.PI * 2); ctx.fill();
  ...
  el("stat-pop").textContent = world.stats.pop; el("stat-alive").textContent = world.stats.alive;
  el("stat-fertile").textContent = world.stats.fertile; el("stat-age").textContent = world.stats.age.toFixed(1);
  el("stat-div").textContent = (world.stats.diversity * 100).toFixed(1) + "%";
  el("stat-f").textContent = world.stats.F.toFixed(3); el("stat-risk").textContent = risk;
  el("stat-lines").textContent = world.stats.lines; el("stat-lost").textContent = world.lostAlleles;
  el("stat-rev").textContent = world.revisions; el("stat-houses").textContent = world.stats.houses ?? "—";
...
document.querySelectorAll("[data-power]").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    if (power === "pause") {
      if (world.event || world.ended) return;
      world.paused = !world.paused; btn.textContent = world.paused ? "▶ Hervat" : "⏸ Pauze";
    }
    if (power === "speed") {
      world.speed = world.speed === 1 ? 3 : world.speed === 3 ? 8 : 1;
      btn.textContent = `⏩ Tempo ×${world.speed}`;
    }
    if (power === "reveal") { world.showKin = !world.showKin; btn.classList.toggle("active", world.showKin); }
  });
});
renderLaws();
function loop(now) {
